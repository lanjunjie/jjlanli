...
public class NewBindingBankActivity extends BaseActivity{
	...
	private TextView new_DBK;
	private TextView new_provinces;
	private TextView new_Openingbank;
	private EditText new_importcard;
	...
	protected void setContentView() {
		setContentView(R.layout.new_binding_bank);
	...
	protected void initViews() {
		setTitle("绑定银行卡");
		findViewAddListener(R.id.linear_new_DBK);
		findViewAddListener(R.id.linear_new_Provinces);
		findViewAddListener(R.id.linear_new_Openingbank);
		findViewAddListener(R.id.butt_Binding_confirm);
		new_DBK = (TextView) findViewById(R.id.tex_new_DBK);
		new_provinces = (TextView) findViewById(R.id.tex_new_provinces);
		new_Openingbank = (TextView) findViewById(R.id.tex_new_Openingbank);
		new_importcard = (EditText) findViewById(R.id.edit_new_importcard);
	...
	protected void onViewClick(View v) {
		switch (v.getId()) {
		case R.id.linear_new_DBK:
			Intent intent = new Intent(NewBindingBankActivity.this,
					BankTypeActivity.class);
			startActivityForResult(intent, 0);
			break;
		case R.id.linear_new_Provinces:
			if (new_DBK.getText().length() == 0) {
		showToast("请填写银行卡类型或银行卡号");
		return;
	} else {
		showSelectDialog(this, "选择所在省市", category1,
				category2);
		System.out.println("category1----------->"+category1);
		System.out.println("category2----------->"+category2);
//		Intent intent2 = new Intent(NewBindingBankActivity.this,
//				ProvinceSelectedActivity.class);
//		startActivityForResult(intent2, 2);
	}
			break;
		case R.id.linear_new_Openingbank:
			if (new_provinces.getText().length() == 0) {
				showToast("请选择省市");
				return;
			} else {
				Intent intent4 = new Intent(NewBindingBankActivity.this,
			...
			break;
		case R.id.butt_Binding_confirm:
			if (new_importcard.getText().toString().length()==0) {
				showToast("请填写完整信息");
				return;
			}else {
				Intent intent5 = new Intent(this,
						NewAffirmMessage.class);
				...
				bundle.putString("AccountAddress", new_Openingbank.getText().toString());
				bundle.putString("NewDBK", new_DBK.getText().toString());
				bundle.putString("ChanneLid",channelid);
				bundle.putString("PayCenterId",paycenterid);
				bundle.putString("IDCard", iDCard);
				bundle.putString("Name", name);
				bundle.putString("NewOpeningbank", new_Openingbank.getText().toString());
				bundle.putString("NewImportcard", new_importcard.getText().toString());
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		case 0:
			if (data != null) {
				...
				new_DBK.setText(res.getChannelname());
		...
		case 1:
			if (data != null) {
				Bundle bundle = data.getExtras();
				new_importcard.setText(new_Openingbank.getText().toString());
			}
		...
		case 2:
			if (data != null) {
				Bundle bundle = data.getExtras();
				new_provinces.setText(bundle.getString("Pname"));
				new_provinces.setText(bundle.getString("Cname"));
			}
		...
		case 4:
			if (data != null) {
				Bundle bundle = data.getExtras();
				new_Openingbank.setText(bundle
				...
				new DialogInterface.OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int which) {
						...
						vRight = right[leftPosition][wheelRight
								.getCurrentItem()];
						new_provinces.setText(vLeft + "-" + vRight);
