...
import com.myfp.myfund.ui.TobeusedFragment.TobeUsedAdapter;
import com.myfp.myfund.ui.TobeusedFragment.TobeUsedAdapter.ViewHolder;

public class HasbeenusedFragment extends BaseFragment{
	...
	class TobeUsedAdapter extends BaseAdapter{
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (convertView==null) {
				...
				convertView = LayoutInflater.from(activity).inflate(R.layout.fragment_hasbeenused, null);
				holder.te_already_money = (TextView) convertView.findViewById(R.id.te_already_money);
				holder.te_already_date = (TextView) convertView.findViewById(R.id.te_already_date);
				convertView.setTag(holder);
			}else {
				holder = (ViewHolder) convertView.getTag();
			}
			TobeUsedResult res = mList.get(position);
				holder.te_already_money.setText("￥"+res.getAmount());
				holder.te_already_date.setText(res.getStartTermination());
				
		return convertView;
		}
		class ViewHolder{
			TextView te_already_money,te_already_date;
		}
